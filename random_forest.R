...
testRows2 <- sample(numRows, trunc(length(numRows) * 0.3))
testData2 <- emotion_extended[testRows,]
trainData2 <- emotion_extended[-testRows,]
...
# random forest 
# ntree = 1000
forests2 <- randomForest(formula = as.factor(emotion) ~ ., ntree = 50, data = trainData2)
# Calm = 2 has the least amount of error at 65% error 
# Sad =  4 has 83% error 
# Happy = 3 82% error
# Angry = 5 71% error 
...
# trying to do a confusion matrix and show accuracy as well 
testData$pred <- predict(forests2, testData2)
testData$pred <- as.factor(testData2$pred)
